    ...
    while True:
        try:
            ...
            for task in task_list:
                data_queue.put(task)
            return data_queue, size,task_list
...
def main():
    ...
    while True:
        ...
        print("Start batch data process...")
        # 获取需要处理的书籍列表
        data_queue, size,data_list = get_batch_task_wait(args.jobID, args.serviceURL)
        # data_queue = query_clickhouse(limit, offset)
        if data_queue is None:
            break
        #创建子线程持续下载数据
        data_dict = {}
        download_thread = downloadThread(data_list, data_dict, args)
        download_thread.daemon = True
        download_thread.start()
        result_queue = queue.Queue(maxsize=size)
        logging.info("Start thread process...")
        print("Start multiTread process...")
        # 多线程处理数据
        threads = []
        for i in range(args.threadNum):
            thread = processThread(i, data_queue, result_queue,data_dict, args)
            thread.start()
