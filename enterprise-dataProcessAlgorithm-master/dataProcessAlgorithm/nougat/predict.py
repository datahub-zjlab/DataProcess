    ...
    for i, (sample, is_last_page) in enumerate(tqdm(dataloader)):
        ...
        for j, output in enumerate(model_output["predictions"]):
            ...
            if is_last_page[j]:
                ...
                page_num = 0
                file_index += 1
            print("missing_page_list", missing_page_list)

    # missing_page_list = ['ananyev2020-6.pdf', 'missingpage_01.pdf']
    # 使用函数
    source_directory = 'input'
    target_directory = 'input_base'
    file_pattern = '.pdf'  # 文件扩展名或其他模式
    print("missing_page_list", missing_page_list)

    copy_files_by_name_pattern(missing_page_list, source_directory, target_directory, file_pattern)
    return target_directory


def base(target_directory):
    args = get_args()
    args.pdf = Path(target_directory).glob('*')
    args.model = "0.1.0-base"
    args.checkpoint = Path(str(args.checkpoint.parent) + '/nougat-0.1.0-base')
    model = NougatModel.from_pretrained(args.checkpoint)
    model = move_to_device(model, bf16=not args.full_precision, cuda=args.batchsize > 0)
    if args.batchsize <= 0:
        # set batch size to 1. Need to check if there are benefits for CPU conversion for >1
