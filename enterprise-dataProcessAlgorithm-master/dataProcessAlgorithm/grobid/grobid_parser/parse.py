...
ns = "http://www.tei-c.org/ns/1.0"  ####tei固定格式前缀
...
def _parse_biblio(elem: ET.Element, ns: str = ns) -> GrobidBiblio:
    ...
    for ela in elem.findall(f".//{{{ns}}}author"):
        a = _parse_author(ela, ns=ns)  ###.full_name###仅需全名
        if a is not None:
            authors.append(a)

    editors = elem.findtext(f".//{{{ns}}}editor")####专著
    ...
    if date_tag is not None:
        ...
        try:
            date_time1 = re.search(r'\b(\d{4})\b', date_tag.text).group(1)
            date_time2 = re.search(r'\b(\d{4})\b', date_tag.attrib.get('when')).group(1)
            if date_time1 and date_time2:
                # print('compare published date ', date_time2, date_time1)
                biblio.date = date_time1 if int(date_time1) > int(date_time2) else date_tag.attrib.get('when')
        except:
            biblio.date = ""
...
####有问题，citation正确，table，figure尚需根据具体嵌入格式调试
def _parse_body(elem: ET.Element, biblio_list: List[GrobidBiblio], ns: str = ns) -> GrobidBody:
    ...
    def get_p_text(node: ET.Element):
        ...
        # Get initial text
        text = node.text if node.text else ""
        # print('check processing para text ', node,node.tag, node.text, node.attrib)
        ...
        for s in node:  ###s句子级
            ...
            if s.tag == f"{{{ns}}}s":
                ...
                for child_node in s:
                    ...
                    text += child_node.tail if child_node.tail else ""
                # Get text that occurs after child nodes
                text += s.tail if s.tail else ""
        # print('check paragraph', text)
        return text
    ...
    for div in el_div:
        ...
        div_text = ''
        for para in div:
            # print('check para text ', para.text)
            para_text = ''
            # print('check para text ', para)
